...
- [ ] Comment di PR test berisi narasi ringkasan per file (bukan cuma risk tag mentah). *(narasi LLM sendiri sudah tervalidasi live lewat `mimo-smoke-test` — lihat 5.3; yang belum adalah post comment-nya ke PR GitHub sungguhan, karena butuh `PR_NUMBER` asli dan ini aksi visible ke orang lain, jadi menunggu konfirmasi eksplisit dulu)*
